                    ...
                    <form action="{{ route('packages.update', $package) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                <option value="cuci_kering" {{ $package->type == 'cuci_kering' ? 'selected' : '' }}>Cuci Kering</option>
                                <option value="cuci_setrika" {{ $package->type == 'cuci_setrika' ? 'selected' : '' }}>Cuci Setrika</option>
                                <option value="express" {{ $package->type == 'express' ? 'selected' : '' }}>Express</option>
                                <option value="dry_clean" {{ $package->type == 'dry_clean' ? 'selected' : '' }}>Dry Clean</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="duration" id="duration" class="form-control @error('duration') is-invalid @enderror" 
                                           value="{{ old('duration', $package->duration) }}" required min="1">
                                    @error('duration')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
